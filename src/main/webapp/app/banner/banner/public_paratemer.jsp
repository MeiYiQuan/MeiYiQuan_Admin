...
<head>
    ...
    // ------------------------------------------------通用部分(开始)--------------------------------------------------------
    	...
    	function add(){
    		 window.location.href ="${pageScope.basePath }message/toadd.do";
    	}
                    ...
                    <div id="tab-1" class="ibox-content">
                        <div class="project-list">
                            <table class="table table-hover" >
                                <tbody id="ttbody">
                                		<tr>
	                                       	<td class="project-title" align="center">公共参数名称</td>
                                	...
                                	<c:forEach items="${requestScope.code.data.list }" var="data">
                                		<tr id="tr${data.id }">
	                                       ...
	                                       <c:if test="${data.type  eq 1 }"> 
	                                        <td class="project-title" align="center">
	                                            <a class="fancybox" href="${data.sys_value }" title="展示图"><img src="${data.sys_value }" style="width:50px;height:50px" /></a>
	                                        </td>
	                                        </c:if>
	                                        
	                                        <c:if test="${data.type  ne 1 }"> 
	                                        <td class="project-title" align="center">
												<small>${data.sys_value }</small>
	                                        </td>
	                                        </c:if>
	                                       
	                                        <td class="project-completion" align="center">
	                                            <a href="${pageScope.basePath }banner/publicInfo.do?id=${data.id}" class="btn btn-white btn-sm"><i class="fa fa-folder"></i>编辑 </a>
	                                        </td>
                                   	    </tr>
                                	</c:forEach>
