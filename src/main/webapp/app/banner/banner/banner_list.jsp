    ...
    // ------------------------------------------------通用部分(开始)--------------------------------------------------------
    	...
    	function sousuo(){
    		...
    		window.location.href = "${pageScope.basePath }banner/banners.do?page=1&size=" + pageEach + 
    				...
    				"&bannerType="+bannerType+"&status="+status+"&type="+type+
    	...
    	// 刷新
    	function refresh(){
    		window.location.href = "${pageScope.basePath }banner/banners.do?page=${requestScope.page.page }&size=" + pageEach + 
    				"&" + "${pageScope.conditions }";
    	}
    	// 发现页面
    	function find(){
    		window.location.href = "${pageScope.basePath }banner/fbanners.do";
    	}
    	// 新增页面
    	function toadd(){
                            ...
                            <div class="col-md-11">
                                <div class="input-group" align="left">
	                                    	banner类型：<select id="bannerType">
		                                    				...
		                                    				<option value="0" <c:if test="${requestScope.bannerType eq '0' }">selected</c:if>>课程</option>
	                                    	...
	                                    	状态：<select id="status">
		                                    				...
		                                    				<option value="2" <c:if test="${requestScope.status eq '2' }">selected</c:if>>禁用</option>
	                                    		   	 </select>
	                                    	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	                                    	展示位置：<select id="type">
		                                    				<option value="" <c:if test="${(requestScope.type eq null) or (requestScope.type eq '') }">selected</c:if>>全部</option>
		                                    				<option value="1" <c:if test="${requestScope.type eq '1' }">selected</c:if>>首页</option>
		                                    				<option value="2" <c:if test="${requestScope.type eq '2' }">selected</c:if>>发现页面</option>
	                                    		   	 </select>
	                                    	<br/><br/>
	                                    	...
	                                    	修改日期：
	                                     	...
	                                     	<!-- 
	                                     		<button type="button" class="btn btn-sm btn-primary" onclick="find()">发现页面</button>
	                                     		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	                                     	 -->
                            ...
                            <table class="table table-hover" >
                                <tbody id="ttbody">
                                		<tr>
	                                        ...
	                                        <td class="project-completion" align="center" >展示位置</td>
	                                        <td class="project-completion" align="center" >跳转类型</td>
                                	...
                                	<c:forEach items="${requestScope.result }" var="banner">
                                		<tr id="tr${banner.id }">
                                			<td class="project-title" align="center">
	                                            <a class="fancybox" href="${banner.pic_save_url }" title="展示图"><img src="${banner.pic_save_url }" style="width:50px;height:50px" /></a>
	                                        </td>
	                                        <td class="project-title" align="center">
	                                        ...
	                                        <td class="project-title" align="center">
												<small>
													<c:if test="${banner.showtype eq '1' }">首页</c:if>
													<c:if test="${banner.showtype eq '2' }">发现页面</c:if>
												</small>
	                                        </td>
	                                        <td class="project-title" align="center">
												<small>
													<c:if test="${banner.jump_type eq '0' }">课程</c:if>
	                                        ...
	                                        <td class="project-title" align="center">
												<small id="status${banner.id}">
													<c:if test="${banner.status eq '2' }">禁用</c:if>
													<c:if test="${banner.status eq '1' }">启用</c:if>
												</small>
	                                        </td>
	                                        <td class="project-completion" align="center">
	                                        	<!-- <a href="javascript:layer.alert('${banner.id }');" class="btn btn-white btn-sm" ><i class="fa fa-pencil"></i>获取ID </a> -->
	                                        	<a href="#" class="btn btn-white btn-sm" onclick="return upStatus('${banner.id}','${banner.name }')" id="ahref${banner.id}"><i class="fa fa-folder"></i>
													...
													<c:if test="${banner.status eq '2' }">启用</c:if>
	                                        	</a>
                                		...
                                		<tr>
                                			<td colspan="10" align="center" >
                                				<qc:qcPage sumPage="${requestScope.page.sumPages }" url="${pageScope.basePath }banner/banners.do" 
