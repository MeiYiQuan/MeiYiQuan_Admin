			...
			this.container.on('click', 'li.js-page-action,li.ui-pager', function(e) {
				...
				if ($(this).hasClass('js-page-action')) {
					...
					if ($(this).hasClass('js-page-next')) {
						_this.current = Math.min(_this.pagecount, Number(_this.current) + 1);
					}
		...
		format: function() {
			...
			if (this.pagecount > 6) {
				...
				if (this.current > 2 && this.current <= this.pagecount - 2) {
					...
					html += '<li data-page="' + (Number(this.current) + 1) + '" class="ui-pager">' + (Number(this.current) + 1) + '</li>';
