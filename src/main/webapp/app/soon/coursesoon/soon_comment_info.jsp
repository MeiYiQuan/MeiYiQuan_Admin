...
<head>
    ...
    // 初始化方法
    	function lahei(){
    		window.location.href = "${pageScope.basePath }sooncourse/reginaComent.do?id=${requestScope.topid}&topid=${requestScope.topid}";
    	}
    	function huifu(){
    		window.location.href = "${pageScope.basePath }sooncourse/blackComent.do?id=${requestScope.topid}&topid=${requestScope.topid}";
    	}
    // ------------------------------------------------通用部分(结束)--------------------------------------------------------
                        ...
                        <form enctype="multipart/form-data" class="form-horizontal m-t" id="commentForm" 
                            ...
                            <div class="form-group">
                             ...
                             <div class="form-group">
                                ...
                                <small>
													<c:if test="${requestScope.status==1 }">已拉黑</c:if>
                                ...
                                <tbody id="ttbody">
                                	...
                                	<c:forEach items="${requestScope.code.data.list }" var="data">
                                		<tr id="tr${data.id }">
	                                       ...
	                                       <td class="project-title" align="center">
												<small>
													<c:if test="${data.status eq '1' }">已拉黑</c:if>
													<c:if test="${data.status eq '2' }">正常</c:if>
												</small>
	                                        </td>
	                                        <td class="project-completion" align="center">
	                                            <a href="${pageScope.basePath }sooncourse/reginaComent.do?id=${data.id}&topid=${requestScope.topid}" class="btn btn-white btn-sm" ><i class="fa fa-pencil"></i>拉黑 </a>
	                                            <a href="${pageScope.basePath }sooncourse/blackComent.do?id=${data.id}&topid=${requestScope.topid}" class="btn btn-white btn-sm" ><i class="fa fa-pencil"></i>恢复 </a>
	                                        </td>
                                   	    </tr>
                                	</c:forEach>
                                    </tbody>
