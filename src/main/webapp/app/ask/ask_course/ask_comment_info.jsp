...
<head>
    ...
    <script type="text/javascript" charset="utf-8" src="${pageScope.basePath }app/UEditor/ueditor.config.js"></script>
	<script type="text/javascript" charset="utf-8" src="${pageScope.basePath }app/UEditor/ueditor.all.min.js"> </script>
	<!--建议手动加在语言，避免在ie下有时因为加载语言失败导致编辑器加载失败-->
	<!--这里加载的语言文件会覆盖你在配置项目里添加的语言类型，比如你在配置项目里配置的是英文，这里加载的中文，那最后就是中文-->
	<script type="text/javascript" charset="utf-8" src="${pageScope.basePath }app/UEditor/lang/zh-cn/zh-cn.js"></script>
    ...
    // 初始化方法
    	function lahei(){
    		window.location.href = "${pageScope.basePath }askcourse/reginaComent.do?id=${requestScope.topid}&topid=${requestScope.topid}";
    	}
    	function huifu(){
                    ...
                    <div class="ibox-content">
                        ...
                        <form enctype="multipart/form-data" class="form-horizontal m-t" id="commentForm" 
                            ...
                            <div class="form-group">
                                <label class="col-sm-3 control-label">求教程名称：</label>
                                <div class="col-sm-8">
                                    <input id="course_name" name="course_name" minlength="2" value="${requestScope.course_name} " placeholder="请输入活动标题" type="text" class="form-control" required="" aria-required="true">
                                </div>
                            </div>
                             <div class="form-group">
                                <label class="col-sm-3 control-label">评论人名称：</label>
                                <div class="col-sm-8">
                                    <input id="user_name" name="user_name" minlength="2" value="${requestScope.user_name }" placeholder="请输入讲师id" type="text" class="form-control" required="" aria-required="true">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <input id="comm_content" name="comm_content" minlength="2" value="${requestScope.comm_content }" type="text" class="form-control" required="" aria-required="true">
                                </div>
                            </div>
                             ...
                             <div class="form-group">
                                ...
                                <small>
													<c:if test="${requestScope.status eq '2' }">正常</c:if>
													<c:if test="${requestScope.status eq '1' }">已被拉黑</c:if>
								</small>
                            </div>
                                		...
                                		<tr id="tr${data.id }">
	                                       ...
	                                       <td class="project-title" align="center">
	                                        ...
	                                        <td class="project-completion" align="center">
	                                            <a href="${pageScope.basePath }askcourse/reginaComent.do?id=${data.id}&topid=${requestScope.topid}" class="btn btn-white btn-sm" ><i class="fa fa-pencil"></i>拉黑 </a>
	                                            <a href="${pageScope.basePath }askcourse/blackComent.do?id=${data.id}&topid=${requestScope.topid}" class="btn btn-white btn-sm" ><i class="fa fa-pencil"></i>恢复 </a>
	                                        </td>
                                   	    </tr>
                                	</c:forEach>
                                    </tbody>
                            ...
                            <div class="form-group">
                                <div class="col-sm-4 col-sm-offset-3">
                               		 <button type="button" class="btn btn-sm btn-primary" onclick="huifu()">恢复</button>
                                     <button class="btn btn-sm btn-primary" type="button" onclick="lahei()">拉黑</button>
                                </div>
                            </div>
