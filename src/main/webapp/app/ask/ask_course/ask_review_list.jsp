...
<head>
    ...
    // ------------------------------------------------通用部分(开始)--------------------------------------------------------
    	...
    	function toadd(){
    		window.location.href = "${pageScope.basePath }askcourse/tosave.do";
    	}
                ...
                <div class="ibox">
                    ...
                    <div id="tab-1" class="ibox-content">
                        <div  class="row m-b-sm m-t-sm">
                            <form id="dataForm" action="/mei_yi_quan_admin/askcourse/getVoteList.do"  method="post">  
                    		...
                    		输入关键字 : <input type="text" id="kaytxt" name="kaytxt" value="${requestScope.kaytxt }" />
                         	...
                         	<span class="col-sm-12" style="margin-top: 10px;">
                         	活 动 状 态  : <select id="status" name="status">
                          				<option value="" <c:if test="${(requestScope.status eq null) or (requestScope.status eq '') }">selected</c:if>>全部</option>
                          				<option value="1" <c:if test="${requestScope.status eq '1' }">selected</c:if>>审核中</option>
                          				<option value="2" <c:if test="${requestScope.status eq '2' }">selected</c:if>>审核失败</option>
                         		   	 	<option value="3" <c:if test="${requestScope.status eq '3' }">selected</c:if>>制作中</option>
                          				<option value="4" <c:if test="${requestScope.status eq '4' }">selected</c:if>>制作失败</option>
                          				<option value="5" <c:if test="${requestScope.status eq '5' }">selected</c:if>>制作完成</option>
                         		   	</select>
