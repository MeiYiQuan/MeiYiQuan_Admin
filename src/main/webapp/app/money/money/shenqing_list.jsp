                ...
                <div class="ibox">
                    <div id="tab-1" class="ibox-content">
                        <div  class="row m-b-sm m-t-sm">
                            <form id="dataForm" action="/mei_yi_quan_admin/money/shenqingList.do"  method="post">  
                        	...
                        	<input type="hidden" name="size" value="${requestScope.pageSize }">
                         	...
                         	申请状态：<select id="status" name="status">
                         				<option value="1" <c:if test="${(requestScope.status eq null) or (requestScope.status eq '') }">selected</c:if>>刚申请</option>
                          				<option value="1" <c:if test="${requestScope.status eq '1' }">selected</c:if>>刚申请</option>
                          				<option value="2" <c:if test="${requestScope.status eq '2' }">selected</c:if>>已通过</option>
                          				<option value="2" <c:if test="${requestScope.status eq '3' }">selected</c:if>>不通过</option>
                         		   	 </select>
                            </span>
                          		 优惠券名称：<input type="text" id="name" name="name" value="${requestScope.name }" />
                                ...
                                <tbody id="ttbody">
                                		<tr>
	                                        <td class="project-title" align="center">申请时间</td>
	                                       	<td class="project-title" align="center">申请人</td>
	                                       	<td class="project-title" align="center">优惠券名称</td>
                                       		<td class="project-title" align="center">优惠券类型</td>
                                    		<td class="project-title" align="center">优惠券描述</td>
	                                       	<td class="project-title" align="center">优惠券面额</td>
                                	...
                                	<c:forEach items="${requestScope.code.data.list }" var="data">
	                                       ...
	                                       <td class="project-completion" align="center">
	                                            <a href="${pageScope.basePath }money/updateShengqin.do?id=${data.wid}&status=2" class="btn btn-white btn-sm">同意 </a>
	                                            <a href="${pageScope.basePath }money/updateShengqin.do?id=${data.wid}&status=3" class="btn btn-white btn-sm">驳回 </a>
	                                        </td>
                                   	    </tr>
                                	</c:forEach>
                                    </tbody>
