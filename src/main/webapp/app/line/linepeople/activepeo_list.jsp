...
<head>
    ...
    // ------------------------------------------------通用部分(开始)--------------------------------------------------------
    	...
    	$(function(){
	    	...
	    	// 新增页面
	   	 function toadd(){
	   		window.location.href = "${pageScope.basePath }active/toAddpeoActive.do?id=${requestScope.id}";
	   	} 
    	
	 	// 刷新
	   	 function refresh(){
	   		window.location.href = "${pageScope.basePath }active/peoActivelist.do?id=${requestScope.id}";
	   	} 
    ...
    // ------------------------------------------------通用部分(结束)--------------------------------------------------------
     ...
     function delect(id){
   	layer.confirm("确定删除此用户吗？",{
   		...
   		window.location.href ="${pageScope.basePath }active/deletejiabin.do?id="+id+"&aid=${requestScope.id}";
   	},function(){
                    ...
                    <div class="ibox-content">
	                                ...
	                                <span class="col-sm-3">
	                                    ...
	                                    <span class="input-group-btn" style="font-size:15px">
		                                    	<button type="button" class="btn btn-sm btn-primary" onclick="refresh()">刷新</button>
				                             	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				                             	<button type="button" class="btn btn-sm btn-primary" onclick="toadd()">添加嘉宾</button>
	                           </span>
                                </div>
                            </div>
                        </form>
                        ...
                        <div class="project-list">
                            <table class="table table-hover" >
                                <tbody id="ttbody">
                                		<tr>
	                                        <td class="project-title" align="center">活动名称</td>
	                                        <td class="project-title" align="center">嘉宾名称</td>
	                                        <td class="project-title" align="center">嘉宾出场费</td>
                                	...
                                	<c:forEach items="${requestScope.code.data.list }" var="data">
                                		<tr id="${data.id }">
	                                        ...
	                                        <td class="project-completion" align="center">
	                                           <a href="${pageScope.basePath }active/jiabinInfo.do?id=${data.id}" class="btn btn-white btn-sm"><i class="fa fa-folder"></i>查看 </a>
	                                           <a href="#" onclick="return delect('${data.id }')" class="btn btn-white btn-sm"><i class="fa fa-folder"></i>删除 </a>
